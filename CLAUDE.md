...
- **HTTP TCP Proxy Directo (v0.0.21):** Se utiliza `API_BASE_URL = "http://yamabiko.proxy.rlwy.net:58436"` (Railway TCP Proxy directo al puerto interno 3000 de Express). Al usar `WiFiClient` (TCP plano sin SSL), la respuesta pasa de 2.5s a 20ms, ahorrando ~35KB de RAM y eliminando por completo las fallas de mbedTLS.
- **Manejo de Errores `-11` vs `-1` (v0.0.20):**
  * **Error `-11` (`HTTPC_ERROR_READ_TIMEOUT`):** Ejecuta un `ESP.restart()` controlado instantáneo de 1.5s. Esto limpia la memoria RAM, los sockets lwIP y el estado `mbedtls` al 100% a nivel hardware, eliminando por completo los bloqueos de 30s del Task Watchdog.
  * **Error `-1` (Error transitorio):** Cierra el socket limpiamente sin apagar la interfaz Wi-Fi física, permitiendo reintentar en el próximo poll sin causar parpadeos en rojo.
