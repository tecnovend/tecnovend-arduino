...
- **HTTPS Keep-Alive & `keepAliveClient.stop()` (OBLIGATORIO en fallas `code <= 0`):** La arquitectura reutiliza el cliente HTTPS (`keepAliveClient` + `keepAliveHttp` con `setReuse(true)`). Ante cualquier error (`code <= 0`), **ES OBLIGATORIO ejecutar `keepAliveClient.stop()`** además de `keepAliveHttp.end()`.
