...
- **Manejo Limpio de Sockets en TCP Proxy (v0.0.22):** Al usar HTTP plano vía TCP Proxy, Railway cierra sockets inactivos a los 5s (retornando `-11`). NUNCA reiniciar el ESP32 ante un `-11`. Simplemente invocar `keepAliveHttp.end()` + `keepAliveClient.stop()` y resetear `keepAliveActive = false` para que la siguiente consulta abra una conexión TCP nueva sin interrumpir el funcionamiento.
